...
class Day23 : Solution<Int, Int>(year = 2021, day = 23) {
    ...
    override fun part1(input: String): Int {
        ...
        val start = Situation(inputLines.subList(1, 4).map { it.toCharArray() }.toTypedArray(), 2, 0)
        return solve(start, 2)
    }

    override fun part2(input: String): Int {
        val inputLines = input.lines()
        val newInput = inputLines.subList(1, 3).toMutableList()
        newInput += "  #D#C#B#A#  "
        newInput += "  #D#B#A#C#  "
        newInput += inputLines.subList(3, 4)
    ...
    private fun solve(startSituation: Situation, n: Int): Int {
        ...
        while (true) {
            ...
            for (hallwayPos in 1..11) {
                ...
                if (!(minOf(hallwayPos, homeRoom) + 1 until maxOf(hallwayPos, homeRoom)).all { x ->
                        situation.config[0][x] == '.'
                    }
                ) {
                    continue
                }
                ...
                if (!(1..n).all {
                        situation.config[it][homeRoom] == '.' ||
                            situation.config[it][homeRoom] == hallwayContent
                    }
                ) {
                    continue
                }
            ...
            for (i in 0..3) {
                for (r in 1..n) {
                    ...
                    for (hallwayPos in 1..11) {
                        ...
                        if (!(minOf(hallwayPos, homeRoom)..maxOf(hallwayPos, homeRoom)).all { cell ->
                                situation.config[0][cell] == '.'
                            }
                        ) {
                            continue
                        }
