...
class Day12 : Solution<Int, Int> {
    ...
    private fun parse(input: String): Map<String, List<String>> = input
        .lines()
        .map { it.split("-") }
    ...
    private fun findPaths(connections: Map<String, List<String>>, twice: Boolean): Int {
        ...
        fun nextPaths(cave: String, visited: Set<String>, twice: Boolean) {
            ...
            for (nextCave in connections[cave]!!) {
                if (nextCave == "end") {
                    paths++
                } else if (nextCave !in v) {
                    nextPaths(nextCave, v, twice)
                } else if (nextCave != "start" && twice) {
                    nextPaths(nextCave, v, false)
                }
            }
        }
