...
class Day18 : Solution<Int, Int> {
    ...
    sealed interface SnailFishNumber {
        ...
        fun reduce(): Map<SnailFishNumber, SnailFishNumber>? = getPair(4)?.let {
            // explode
        ...
        operator fun plus(other: SnailFishNumber) = generateSequence<SnailFishNumber> (SnailFishPair(this, other)) { n -> n.reduce()?.let { n.apply(it) } }.last()
    }
    ...
    private fun parse(input: String): List<SnailFishNumber> = input.lines().map { parseSnailFishNumber(it) }

    private fun parseSnailFishNumber(input: String): SnailFishNumber = if (input.first().isDigit()) {
        SnailFishInt(input.first().digitToInt())
    } else {
        var count = 0
        val content = input.removeSurrounding("[", "]")
        val (s1, s2) = content.foldIndexed("") { i, acc, c ->
            when (c) {
                '[' -> count++.let { acc }
                ']' -> count--.let { acc }
